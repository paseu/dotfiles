...
chrome.extension.onMessage.addListener(
    function (request, sender, sendResponse) {
        ...
        switch (request.type) {
            case "getTabData":
                sendResponse(topDB.getDataPerTabId(request.tabid));
                break;
            case "count":
                chrome.storage.sync.get("optedout", function (obj) {
                    if (!obj["optedout"])
                        $.ajax({
                            type: "GET",
                            url: "http://crxmouse.com/count.php?url=" + encodeURIComponent(request.data.url) + "&event=" + request.data.event + "&action=" + request.data.action,
                        });
                });
                break;
            case "gesture":
