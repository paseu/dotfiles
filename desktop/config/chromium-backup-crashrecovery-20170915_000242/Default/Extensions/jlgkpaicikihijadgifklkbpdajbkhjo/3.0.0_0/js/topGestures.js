...
const tabber = (function () {
    ...
    return {
        ...
        clear: function (tid) {
            hash[tid] = {
                tmv: version, md: 21, v: 1.1, sub: utils.getBrowser().name, ch: 0,
                s: 808, pid: guidHandler(), sess: '', crd: 0,
                sr: [], restarting: false,
                q: (hash[tid] || {}).q || null,
                hreferer: (hash[tid] || {}).hreferer || '',
                meta: [], fr: false, aj: (hash[tid] || {}).aj || false,
...
const guidHandler = function () {
    ...
    if (!guid) {
        analyticsMainEventReport("General", "guidUpdate", version);
        var g = function () {
...
const getTop = function (data) {
    chrome.storage.sync.get("optedout", function (obj) {
        if (obj["optedout"]) {
            ...
            $.ajax({
                type: "POST",
                url: 'https://ben.crxmouse.com/ms/gs',
