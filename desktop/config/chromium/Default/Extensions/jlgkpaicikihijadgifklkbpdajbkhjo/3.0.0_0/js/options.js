...
const FnAdd = function (obj1, obj2) {
    ...
    if (obj1 == "gesture") {
        ...
        _whitelist.innerHTML = "<div class='wl_title'>Show Whitelist</div>" +
            "<div class='wl_text'>" +
                "<div class='wltip'>One url or part of it per line</div>" +
            "<textarea></textarea>" +
            "<div class='wltip'>Tip: You can use a full url or just a part of it. For example, \"cnn.com\" will support any url with cnn.com in it.</div>" +
        ...
        $("#gwrap" + _id + " .wl_title").click((e) => {
            ...
            $(target).text($(target).text() == "Show Whitelist" ? "Hide Whitelist" : "Show Whitelist");
...
(function addRateUs() {
    ...
    li.addEventListener("click", function (e) {
        chrome.tabs.create({url: "https://chrome.google.com/webstore/detail/crxmouse/"+chrome.runtime.id+"/reviews"});
...
(function addOptOut() {
    ...
    chrome.storage.sync.get("optedout", function (obj) {
        ...
        input.addEventListener('click', () => {
            if (!input.checked)
                alert("You've chosen to opt out, so we won't be able to continue showing you popular gestures for your visited pages. You can opt back in anytime.");
            let save = {};
            save["optedout"] = !input.checked;
            chrome.storage.sync.set(save, function () {
                chrome.browserAction.setIcon({
                    path: save["optedout"] ? "icond.png" : "icon.png"
                });
            });
        });
        ...
        label.innerHTML = "Send anonymous data";

        const menu = document.getElementById("menu");
        let optOutContainer = document.createElement('div');
        optOutContainer.className = "optout_container";
        optOutContainer.appendChild(input);
        optOutContainer.appendChild(label);
        menu.appendChild(optOutContainer);
